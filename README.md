...
| `/admin` | Open demo CMS, no login. Linked from the nav with a "Demo" pill. Edit the menu intro and each dish (name, category, price, description), add, reorder and delete. A new dish is inserted after the last dish of its own category, then scrolled to and focused, so it cannot land off screen. Everything runs on a draft held in `AdminApp` state only: nothing persists and `/menu` renders `seedMenu` from `lib/menu.ts`, so the public site cannot be changed from here. Every action raises a "Demo only, changes aren't saved" toast. Also manages the menu photography: each category's carousel photos can be reordered, given new alt text, removed, or added by uploading from the device (held as blob URLs in the tab, revoked on unmount). |
